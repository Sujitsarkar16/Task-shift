  ...
  users: {
    ...
    validator: {
      ...
      properties: {
        ...
        name: optionalStringField(160),
        displayName: optionalStringField(160),
        email: optionalStringField(240),
        emailVerified: { bsonType: ['date', 'null'] },
        image: optionalStringField(2000),
        photoURL: optionalStringField(2000),
        theme: { enum: ['light', 'dark'] },
        defaultView: optionalStringField(40),
        reminderLeadDays: { bsonType: ['int', 'long', 'double', 'decimal', 'null'] },
      }
    },
    indexes: [
      {
      ...
      { key: { email: 1 }, name: 'email_unique', unique: true, partialFilterExpression: { email: { $exists: true, $type: 'string' } } },
    ],
    normalizeCreate(input) {
      const payload = asPlainObject(input, 'user');
      return {
        userId: asOptionalString(payload.userId, 'userId', 120),
        name: asOptionalString(payload.name, 'name', 160),
        displayName: asOptionalString(payload.displayName, 'displayName', 160),
        email: asOptionalString(payload.email, 'email', 240),
        image: asOptionalString(payload.image, 'image', 2000),
        ...
        reminderLeadDays: asOptionalNumber(payload.reminderLeadDays, 'reminderLeadDays', { min: 1, max: 90, integer: true }),
      };
    },
    normalizeUpdate(input) {
      ...
      if (hasOwn(payload, 'name')) {
        const value = asOptionalString(payload.name, 'name', 160);
      ...
      if (hasOwn(payload, 'image')) {
        const value = asOptionalString(payload.image, 'image', 2000);
        if (value) set.image = value;
        else unset.push('image');
      }
      ...
      if (hasOwn(payload, 'reminderLeadDays')) {
        const val = asOptionalNumber(payload.reminderLeadDays, 'reminderLeadDays', { min: 1, max: 90, integer: true });
        if (val !== undefined) set.reminderLeadDays = val;
        else unset.push('reminderLeadDays');
      }
